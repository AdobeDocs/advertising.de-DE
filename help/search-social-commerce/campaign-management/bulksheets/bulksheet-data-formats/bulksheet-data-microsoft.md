...
| [!UICONTROL Auto Target Expression] | Dynamische Suchziele für die Anzeigengruppe. Verwenden Sie für alle Ziele &quot;[!UICONTROL All Web Pages].“<br><br>Um bis zu drei dynamische Suchkriterien auszuwählen, verwenden Sie den `<category>=<target>`, wobei &lt;category> jede der folgenden Kategorien enthalten kann. Mehrere Ziele für eine einzelne Kategorie mit &quot;`[blank space] and [blank space]`&quot; verbinden und mehrere Kategorien mit &quot;`[blank space] and [blank space]`&quot; verbinden.<br><ul><li><i>Kategorie:</i> So zeigen Sie dynamische Suchanzeigen für indizierte Seiten mit einer bestimmten Google-Inhaltskategorie an.</li><li><i>URL:</i> um dynamische Suchanzeigen für indizierte Seiten mit einer bestimmten URL anzuzeigen, wobei der Wert an einer beliebigen Stelle in der URL enthalten sein kann.</li><li><i>Seitentitel:</i> Anzeigen dynamischer Suchanzeigen für indizierte Seiten mit bestimmtem Text im Seitentitel.</li><li><i>Seiteninhalt:</i> Anzeigen dynamischer Suchanzeigen für indizierte Seiten mit bestimmtem Inhalt.</li></ul>Beispiel: url=shoes.example.com und page title=<br>-Beispiel: Alle Web-Seiten |
...
| [!UICONTROL Parent Product Groupings] | Die Hierarchie übergeordneter Produktgruppen.<br><br>Beispiel: `All Products>>ProductTypeL1=a>>ProductTypeL2=b` |
...
| [!UICONTROL RLSA Target ID] | Die eindeutige ID, die ein vorhandenes RLSA-Ziel auf Kampagnen- oder Anzeigengruppenebene identifiziert. In CSV- und TSV-Dateien muss vor ihr ein einfaches Anführungszeichen (&#39;).[^1 stehen. ] nur erforderlich, wenn Sie die Zielgruppe oder den Ausschluss ändern oder löschen, es sei denn, die Zeile enthält eine &quot;[!UICONTROL AMO ID]&quot; für die Zielgruppe. |
| [!UICONTROL AMO ID] | (In generierten Bulksheets) Eine von Adobe generierte eindeutige Kennung für eine synchronisierte Entität. Bei responsiven Suchanzeigen ist die AMO-ID erforderlich, um Anzeigen zu bearbeiten oder zu löschen, es sei denn, Sie beziehen die Anzeigen-ID ein. Um Daten für alle anderen Entitätstypen mit einer AMO ID zu bearbeiten, muss die AMO ID die Daten bearbeiten oder löschen, es sei denn, Sie beziehen die Entitäts-ID und die übergeordnete Entitäts-ID ein.<br><br>Search, Social und Commerce verwenden den Wert, um die richtige Identität zur Bearbeitung zu bestimmen, veröffentlichen die ID jedoch nicht im Anzeigennetzwerk. |
...
| [!UICONTROL AMO ID] | Erforderlich zum Bearbeiten oder Löschen der Daten, es sei denn, Sie beziehen die Entitäts-ID und die ID der übergeordneten Entität ein.<br><br>Suche, Social und Commerce verwenden den Wert, um die richtige zu bearbeitende Identität zu bestimmen, senden die ID jedoch nicht an das Werbenetzwerk. |
...
| [!UICONTROL AMO ID] | Erforderlich zum Bearbeiten oder Löschen der Daten, es sei denn, Sie beziehen die Werbe-ID ein.<br><br>Search, Social und Commerce verwenden den Wert, um die richtige Identität zum Bearbeiten zu bestimmen, senden die ID jedoch nicht an das Werbenetzwerk. |
...
| [!UICONTROL Campaign ID] | optional |
| [!UICONTROL Sitelink ID] | Nur erforderlich, wenn Sie den Sitelink ändern oder löschen, es sei denn, die Zeile enthält a) ausreichend Eigenschaftenspalten, um den Sitelink zu identifizieren, oder b) ein &quot;[!UICONTROL AMO ID]&quot;. Wenn Sie jedoch weder [!UICONTROL Sitelink ID] noch [!UICONTROL AMO ID] einbeziehen und die Eigenschaftsspalten mit mehreren Sitelinks übereinstimmen, ändert sich der Status für nur einen der Sitelinks.<br><br><b>Hinweis:</b> Wenn Sie die Sitelink-Eigenschaftsspalten mit Ausnahme des Status für einen vorhandenen Sitelink bearbeiten und weder die [!UICONTROL Sitelink ID] noch die [!UICONTROL AMO ID] einbeziehen, wird ein neuer Sitelink erstellt und der vorhandene Sitelink wird nicht geändert. |
| [!UICONTROL AMO ID] | Erforderlich zum Bearbeiten oder Löschen der Daten, es sei denn, Sie beziehen die Entitäts-ID und die ID der übergeordneten Entität ein.<br><br>Suche, Social und Commerce verwenden den Wert, um die richtige zu bearbeitende Identität zu bestimmen, senden die ID jedoch nicht an das Werbenetzwerk. |
...
| [!UICONTROL Campaign ID] | optional |
| [!UICONTROL AMO ID] | Erforderlich zum Bearbeiten oder Löschen der Daten, es sei denn, Sie beziehen die Kampagnen-ID ein<br><br>Search, Social und Commerce verwenden den Wert zum Bestimmen der richtigen Identität zum Bearbeiten, veröffentlichen die ID jedoch nicht im Werbenetzwerk. |
...
| [!UICONTROL AMO ID] | Erforderlich zum Bearbeiten oder Löschen der Daten, es sei denn, Sie beziehen die Gerätezielgruppen-ID ein.<br><br>Suche, Social und Commerce verwenden den Wert, um die richtige zu bearbeitende Identität zu bestimmen, senden die ID jedoch nicht an das Werbenetzwerk. |
...
| [!UICONTROL AMO ID] | Erforderlich zum Bearbeiten oder Löschen der Daten, es sei denn, Sie beziehen die RLSA Target ID ein.<br><br>Search, Social und Commerce verwenden den -Wert, um die richtige zu bearbeitende Identität zu bestimmen, senden die ID jedoch nicht an das Werbenetzwerk. |

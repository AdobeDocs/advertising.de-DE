---
title: (Neue Benutzeroberfläche) Über die [!UICONTROL Keywords]
description: Erfahren Sie, was Sie in der [!UICONTROL Keywords] tun können.
feature: Search Optimization, Search Campaign Management
hide: true
exl-id: e89e47d3-b41c-4330-8b72-f601194d5ddc
source-git-commit: d375af35af1db5aab75db2e712ae54a39f392c3d
workflow-type: tm+mt
source-wordcount: '75'
ht-degree: 0%

---

# (Neue Benutzeroberfläche) Über die [!UICONTROL Keywords]

*Beta-Funktion*

Die Ansicht [!UICONTROL Manage] > [!UICONTROL Keywords] umfasst zwei Registerkarten:

* Auf der Registerkarte [!UICONTROL Keywords] werden alle Keywords in der gefilterten Ansicht für das ausgewählte Advertiser-Konto aufgelistet.

* Die Registerkarte [!UICONTROL Negatives] listet alle negativen Keywords in der gefilterten Ansicht für das ausgewählte Advertiser-Konto auf.

## Verfügbare Aktionen

* [Zuweisen von Einschränkungen zu Keywords und Aufheben der Zuweisung von Einschränkungen zu Keywords](/help/search-social-commerce/new-ui/target/keywords/keyword-constraint-assignments-manage.md)

* [Zuweisen von ](/help/search-social-commerce/new-ui/reports/label-classifications-manage.md) zu Schlüsselwörtern

>[!MORELIKETHIS]
>
>* [Verwalten von Einschränkungszuweisungen für Schlüsselwörter](/help/search-social-commerce/new-ui/target/keywords/keyword-constraint-assignments-manage.md)
